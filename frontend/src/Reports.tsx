...
export type ShiftRow = { id:number;date:string;grade:'M500'|'M600';bags:number;tons:number;loadingTons:number;loadingMaterial?:'SAND'|'GRAVEL';packagingPay:number;loadingPay:number;barrel:{name:string};workers:{worker:{name:string};salary:number}[] };
type Sale = { id:number;date:string;grade?:'M500'|'M600';client?:string;bags?:number;material?:'SAND'|'GRAVEL';tons?:number;pricePerTon?:number;concreteGrade?:string;address?:string;vehicle?:string;volume?:number;paid?:boolean;amount:number;kind?:'cement'|'concrete' };
...
export function Operations({kind,reload}:{kind:'sales'|'materials'|'expenses';reload:()=>void}) {
 ...
 const editMaterial=async(x:Sale)=>{const tons=prompt('Новое количество тонн',String(x.tons??0));if(tons===null)return;const pricePerTon=prompt('Новая цена за тонну, ₽',String(x.pricePerTon??0));if(pricePerTon===null)return;const selected=prompt('Материал: SAND — песок, GRAVEL — щебень',x.material??'SAND');if(selected===null)return;await api(`/materials/${x.id}`,{method:'PATCH',body:JSON.stringify({date:x.date.slice(0,10),material:selected.toUpperCase(),tons,pricePerTon})});load();reload()};
 return <><h2>История операций</h2><div className="list">{rows.map(x=><div className="history compact" key={`${(x as Sale).kind||kind}-${x.id}`}><div><strong>{(x as Sale).kind==='concrete'?`Бетон ${(x as Sale).concreteGrade} · ${(x as Sale).address}`:'client'in x&&x.client?`${x.client}${x.grade?` · ${x.grade}`:''}`:'material'in x?(x.material==='SAND'?'Песок':'Щебень'):(x as ExpenseRow).comment||category((x as ExpenseRow).category)}</strong><small>{date(x.date)} · {(x as Sale).kind==='concrete'?`${Number((x as Sale).volume).toFixed(2)} м³ · авто: ${(x as Sale).vehicle||'не указано'}`:'bags'in x&&x.bags?`${x.bags} меш.`:'tons'in x&&x.tons?`${Number(x.tons).toFixed(2)} т`:category((x as ExpenseRow).category)}</small></div><b>{rub(Number(x.amount))}</b>{kind==='materials'&&<button className="icon-button" onClick={()=>editMaterial(x as Sale)} title="Изменить продажу"><Pencil/></button>}{kind==='expenses'&&<button className="icon-button" onClick={()=>editExpense(x as ExpenseRow)} title="Изменить расход"><Pencil/></button>}<button className="icon-button danger" onClick={()=>remove(x)}><Trash2/></button></div>)}{!rows.length&&<Empty/>}</div></>;
}

export function WorkerStats() { const [period,setPeriod]=useState('month'),[rows,setRows]=useState<WorkerStat[]>([]);const load=()=>api<WorkerStat[]>(`/workers/stats?period=${period}`).then(setRows);useEffect(()=>{load()},[period]);const reset=async()=>{if(!confirm('Сбросить текущие показатели зарплаты на ноль? История смен сохранится.'))return;await api('/salary/reset',{method:'POST',body:'{}'});load()};return <><PeriodTabs value={period} onChange={setPeriod}/><button className="reset-salary" onClick={reset}>Сбросить зарплату</button><small className="cash-note">Начнётся новый период зарплаты. Смены и старая аналитика не удалятся.</small><div className="list salary-list">{rows.map(x=><div className="worker-stat" key={x.id}><div><strong>{x.name}</strong><small>{x.position}</small></div><b>{rub(x.earnings)}</b><div className="stat-line"><span>{x.workDays} рабочих дней</span><span>{x.shifts} смен</span><span>{x.tons.toFixed(2)} т</span></div></div>)}</div></> }
...
export function Analytics(){
  ...
  const income=days.map(day=>byDay(data.cement,day,x=>Number(x.amount))+byDay(data.concrete.filter(x=>x.paid),day,x=>Number(x.amount)));
