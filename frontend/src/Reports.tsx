...
export function Production({reload}:{reload:()=>void}) {
  ...
  return <><div className="summary"><Factory/><span>Произведено всего</span><strong>{bags} меш. · {tons.toFixed(2)} т</strong></div><h2>История смен</h2><div className="list">{rows.map(x=><div className="history" key={x.id}><div className="history-top"><div><strong>{date(x.date)} · {x.grade} · {x.bags} мешков</strong><small>{Number(x.tons).toFixed(3)} т · {x.barrel.name}</small></div><button className="icon-button danger" onClick={()=>remove(x.id)}><Trash2/></button></div><div className="chips">{x.workers.map(w=><span key={w.worker.name}>{w.worker.name} · {rub(Number(w.salary))}</span>)}</div></div>)}{!rows.length&&<Empty/>}</div></>;
}
