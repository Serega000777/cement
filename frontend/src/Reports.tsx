...
type Sale = { id:number;date:string;grade?:'M500'|'M600';client?:string;bags?:number;material?:'SAND'|'GRAVEL';tons?:number;concreteGrade?:string;address?:string;vehicle?:string;volume?:number;paid?:boolean;amount:number;kind?:'cement'|'concrete' };
...
export function Operations({kind,reload}:{kind:'sales'|'materials'|'expenses';reload:()=>void}) {
 ...
 return <><h2>История операций</h2><div className="list">{rows.map(x=><div className="history compact" key={`${(x as Sale).kind||kind}-${x.id}`}><div><strong>{(x as Sale).kind==='concrete'?`Бетон ${(x as Sale).concreteGrade} · ${(x as Sale).address}`:'client'in x&&x.client?`${x.client}${x.grade?` · ${x.grade}`:''}`:'material'in x?(x.material==='SAND'?'Песок':'Щебень'):(x as ExpenseRow).comment||category((x as ExpenseRow).category)}</strong><small>{date(x.date)} · {(x as Sale).kind==='concrete'?`${Number((x as Sale).volume).toFixed(2)} м³ · авто: ${(x as Sale).vehicle||'не указано'}`:'bags'in x&&x.bags?`${x.bags} меш.`:'tons'in x&&x.tons?`${Number(x.tons).toFixed(2)} т`:category((x as ExpenseRow).category)}</small></div><b>{rub(Number(x.amount))}</b>{kind==='expenses'&&<button className="icon-button" onClick={()=>editExpense(x as ExpenseRow)} title="Изменить расход"><Pencil/></button>}<button className="icon-button danger" onClick={()=>remove(x)}><Trash2/></button></div>)}{!rows.length&&<Empty/>}</div></>;
}
